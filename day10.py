...
different ways they can be arranged. Every arrangement needs to connect the charging outlet to your device. 
...
def read_input(file):
    with open(file, 'r') as f:
        return parse_data(f.read().splitlines())

def parse_data(data):
    prepared_data = []
    prepared_data.append(0)
    [prepared_data.append(int(x.strip())) for x in data]
    prepared_data.append(max(prepared_data) + 3)
    return sorted(prepared_data)

def count_adapters(low, high, data):
...
jolts = read_input('./data/10_input.txt')
print(f'Part 1: {count_adapters(0, 0, jolts)}')

placeholder = [1] + [0 for i in range(len(jolts) - 1)]
for i in range(len(jolts)):
    for j in range(1, 4):
        if jolts[i] + j in jolts:
            placeholder[jolts.index(jolts[i] + j)] += placeholder[i]

print(f'Part 2: {placeholder[-1]}')
