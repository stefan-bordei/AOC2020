...
inputFile = open("./data/9_input.txt", 'r')
content = inputFile.read().strip().splitlines()

preamble = []
found = False
fault = 0

for i in range(25, len(content) - 1):
    preamble = content[i-25: i]
    for elem in preamble:
        target = int(content[i]) - int(elem)
        if str(target) in preamble and target != int(elem):
            found = True
    if not found:
        fault = int(content[i])
        print("Part 1:", fault)
...
# preamble and fault stored

for iter1, elem1 in enumerate(content):
    elemList = []
    if int(elem1) >= fault:
        continue
    elementSum = int(elem1)
    elemList.append(elem1)
    if iter1+1 > len(content):
        break
    for iter2, elem2 in enumerate(content[iter1+1:]):
        elementSum += int(elem2)
        elemList.append(elem2)
        if elementSum == fault:
            sortedList = sorted(elemList, key=lambda i: int(i))
            print(f'Part 2: {int(sortedList[0]) + int(sortedList[-1])}')
            break
        if elementSum > fault:
            break
