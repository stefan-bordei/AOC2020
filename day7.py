...
def hasShinyGold(colour):
    if colour == "shiny gold": 
        return True
    else:
        return any(hasShinyGold(c) for _, c in bags[colour] )


def countBags(bag_type):
    return 1 + sum(int(number)*countBags(colour) for number, colour in bags[bag_type])


inputFile= open("./data/7_input.txt", "r")
