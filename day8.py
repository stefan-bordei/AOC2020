...
inputFile = open("./data/8_input.txt", 'r')
content = inputFile.read().splitlines()


def check_run(commands):
    acc = 0
    visited = []
    i = 0
    ...
    while i < len(commands):    
        ...
        visited.append(i)
        operation, value = commands[i].split(' ')
...
print(f"Part1\nAccumulator before loop: {check_run(content)[0]}")

for i in range(len(content)):
    if 'jmp' in content[i]:
        content[i] = content[i].replace('jmp', 'nop')
        acc, loop = check_run(content)

        if loop:
            print(f"Part2\nAccumulator after loop fix: {acc}")
            break
        else:
